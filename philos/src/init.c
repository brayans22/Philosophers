...
static int init_philos(t_philo **philos, long total_philos, t_fork *forks, t_program *program)
{
    ...
    *philos = get_malloc_memory(sizeof(t_philo) * total_philos);
    ...
    while (++i < total_philos)
    {
        ...
        if (set_mutex_status(&(*philos)[i].data_philo, INIT_MTX) != OK)
            return (ERROR);
    }
    return (OK);
}
...
static int wrapper_init_program(t_program *program)
{
    ...
    if (set_mutex_status(&program->routine_mutex, INIT_MTX) != OK)
        return (set_mutex_status(&program->end_mutex, DESTROY_MTX) \
        ,set_mutex_status(&program->print_mutex, DESTROY_MTX), ERROR);
    if (set_mutex_status(&program->eat_mutex, INIT_MTX) != OK)
        return (set_mutex_status(&program->end_mutex, DESTROY_MTX) \
        ,set_mutex_status(&program->print_mutex, DESTROY_MTX)
        ,set_mutex_status(&program->routine_mutex, DESTROY_MTX), ERROR);
